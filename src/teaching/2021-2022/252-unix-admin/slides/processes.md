...
## UNIX Boot
- Once control is handled over the kernel, the first process is *init* is created.
  - PID 0 or 1 (System Dependent)
...
## ps command
- Useful `ps` options
  - *Different Options* - UNIX, BSD, GNU
  - No Arguments - Shows processes of current user and terminal
  - `ps -A` or `ps -e` - Every Active Process
  - `ps -f` - Full format
  - `ps -x` - Commands ran by the user running *ps* (Usually you)
  - `ps -f -u <login>` - Full Format output of processes owned by given login name
...
## Process States
![bg 80%](./images/process_states.jpg)
...
- Magic Number
  ...
  - `file` - Useful for identifying the type of file
